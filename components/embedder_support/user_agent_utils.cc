...
// Function to parse the string of UACH data
std::string parseStr(std::string uachStr, UACHSpoofType SpoofTarget) {
    std::vector<std::string> fields = split(uachStr, ";;;");
...
const blink::UserAgentBrandList GetUserAgentBrandList(
  ...
  std::string uachstr = parseFile();
  custom_brand = parseStr(uachstr, UACHCustomName);
...
#endif
  std::string custom_brand_version = parseStr(uachstr, UACHCustomVersion);
...
blink::UserAgentMetadata GetUserAgentMetadata(bool only_low_entropy_ch) {
  ...
  std::string is_mobile = parseStr(uachstr, UACHCustomIsMobile);
  if(is_mobile == std::string("true"))
	  metadata.mobile = true;
  else
      metadata.mobile = false;
  std::string is_platform;
  is_platform = parseStr(uachstr, UACHCustomPlatform);
  ...
  full_version_custom = parseStr(uachstr, UACHCustomFullVersion);
  ...
  architecture_custom = parseStr(uachstr, UACHCustomArchitecture);
  ...
  platform_version_custom = parseStr(uachstr, UACHCustomPlatformVersion);
